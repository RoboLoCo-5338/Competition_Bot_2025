...
public abstract class AdvantageScopeSubsystem<T extends IO, I extends IOInputs & LoggableInputs> extends SubsystemBase implements AutoCloseable {
    ...
    protected void addDisconnectedAlerts() {
        for (int i = 0; i < ios.size(); i++) {
            disconnectedAlerts.add(new Alert("Device " + i + " of Subsystem" + getName() + " is disconnected.", Alert.AlertType.kError));
        }
    }
}
